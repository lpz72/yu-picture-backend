...
import org.lpz.yupicture.shared.websocket.disruptor.PictureEditEventProducer;
import org.lpz.yupicture.shared.websocket.model.PictureEditActionTypeEnum;
    ...
    public void handleEnterEditMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        ...
        if (!pictureEditingUsers.containsKey(pictureId)) {
            ...
            if (!editingUserId.equals(user.getId())) {
                ...
                // 获取到正在编辑的用户
                User editingUser = userApplicationService.getUserById(editingUserId);

                PictureEditResponseMessage pictureEditResponseMessage = new PictureEditResponseMessage();
                pictureEditResponseMessage.setType(PictureEditMessageTypeEnum.INFO.getValue());
                pictureEditResponseMessage.setMessage(String.format("用户 %s 正在编辑图片，请稍后重试", editingUser.getUserName()));
                pictureEditResponseMessage.setUser(userApplicationService.getUserVO(editingUser));
